...
const SLACK_READ_TOOL_NAMES = [
  ...
  // Likewise, change the repository of the document backing this channel.
  "mcp__gdocs__set_channel_repository"
];
...
async function runClaudeResearchAgentOnce(
  ...
  const setChannelRepositoryTool = tool(
    "set_channel_repository",
    "Link the rdocs document backing THIS Slack channel to a GitHub or HuggingFace repository. Pass a repository URL and optional branch, or pass \"none\" as repository to disconnect it. The server enforces that the requesting user has edit access to the channel's document.",
    {
      repository: z.string().min(1).max(500).describe('Repository URL, or "none" to disconnect.'),
      branch: z.string().max(120).optional().describe("Optional Git branch; omit it to use the default branch.")
    },
    async (args) => callSlackTool("set_channel_repository", args)
  );
