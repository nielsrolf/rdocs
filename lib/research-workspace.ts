...
// Widget builds run untrusted code, so they now live in agent-core/ (the
// container entrypoint runs them in-sandbox). Re-exported here to preserve the
// `@/lib/research-workspace` import path used by the in-process fallback and the
// manual widget-refresh routes.
export { parseBuildCommand, validateWidgetBuildCommand, runWidgetBuild } from "../agent-core/widget-build";
